...
public final class Lz4RawCompressor {
	...
	private static int compress0(final Object inputBase, final long inputAddress,
		...
		if (maxOutputLength < maxCompressedLength(inputLength)) {
			throw new IllegalArgumentException("Max output length must be larger than "
					+ maxCompressedLength(inputLength));
		}
		...
		do {
			...
			while (true) {
				...
				int matchLength = count(inputBase, input + MIN_MATCH, matchIndex + MIN_MATCH,
						matchLimit);
				output = emitMatch(outputBase, output, tokenAddress,
						(short) (input - matchIndex), matchLength);
	...
	private static int count(Object inputBase, final long start, long matchStart,
			long matchLimit) {
		long current = start;
		...
		if (current < matchLimit - (SIZE_OF_INT - 1)
				&& UnsafeUtil.getInt(inputBase, matchStart) == UnsafeUtil.getInt(inputBase, current)) {
			current += SIZE_OF_INT;
