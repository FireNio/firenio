...
public final class Lz4RawDecompressor {
	...
	private static int decompress0(final Object inputBase, final long inputAddress,
		...
		if (outputAddress == outputLimit) {
			if (inputLimit - inputAddress == 1 && UnsafeUtil.getByte(inputBase, inputAddress) == 0) {
				return 0;
			}
			return -1;
		}
		...
		while (input < inputLimit) {
			...
			if (literalOutputLimit > (fastOutputLimit - MIN_MATCH)
					|| input + literalLength > inputLimit
							- (OFFSET_SIZE + TOKEN_SIZE + LAST_LITERAL_SIZE)) {
			...
			if (matchOutputLimit > fastOutputLimit - MIN_MATCH) {
				if (matchOutputLimit > outputLimit - LAST_LITERAL_SIZE) {
					throw new MalformedInputException(input - inputAddress, String
							.format("last %s bytes must be literals", LAST_LITERAL_SIZE));
				}
