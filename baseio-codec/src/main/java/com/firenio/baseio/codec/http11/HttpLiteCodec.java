 ...
 *
 */
public class HttpLiteCodec extends ProtocolCodec {
    ...
    public HttpLiteCodec(String server) {
        this(server, 0);
    }

    public HttpLiteCodec(String server, int frameBuffer) {
    ...
    public Frame decode(NioSocketChannel ch, ByteBuf src) throws IOException {
        ...
        if (decode_state == decode_state_header) {
            for (;;) {
                ...
                if (size == 0) {
                    if (f.getHeaderLength() < 1) {
                        decode_state = decode_state_complate;
                    } else {
                        if (f.getHeaderLength() > bodyLimit) {
                            throw new IOException("over limit:" + f.getHeaderLength());
                        }
    ...
    private void parseRequestURL(HttpFrameLite f, int skip, CharSequence line) {
