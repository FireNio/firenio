...
public class LinkAbleByteBufAllocator extends AbstractLifeCycle
        implements ByteBufAllocator, Linkable {
    ...
    public LinkAbleByteBufAllocator(ByteBufAllocator allocator, int index) {
        this.index = index;
    ...
    public ByteBuf allocate(int capacity, LinkAbleByteBufAllocator allocator) {
        if (allocator == this) {
            //FIXME 是否申请java内存
            return UnpooledByteBufAllocator.getHeap().allocate(capacity);
        }
        ByteBuf buf = unwrap().allocate(capacity);
        if (buf == null) {
