 ...
 *
 * @deprecated Use {@link SslContextBuilder} to create {@link JdkSslContext}
 *             instances and only use {@link JdkSslContext} in your code.
 */
@Deprecated
public final class JdkSslClientContext extends JdkSslContext {
	 ...
	 *
	 * @param certChainFile
	 *             an X.509 certificate chain file in PEM format. {@code null}
	 ...
	 *             the {@link TrustManagerFactory} that provides the
	 *             {@link TrustManager}s that verifies the certificates sent
	 *             from servers. {@code null} to use the default.
	 * @deprecated use {@link SslContextBuilder}
	 */
	@Deprecated
	...
	/**
	 * @deprecated Use the constructors that accept
	 *             {@link ApplicationProtocolConfig} or
	 ...
	 * @param nextProtocols
	 *             the application layer protocols to accept, in the order of
	 *             preference. {@code null} to disable TLS NPN/ALPN extension.
	 ...
	 */
	@Deprecated
	public JdkSslClientContext(File certChainFile, TrustManagerFactory trustManagerFactory, Iterable<String> ciphers,
			Iterable<String> nextProtocols, long sessionCacheSize, long sessionTimeout) throws SSLException {
		this(certChainFile, trustManagerFactory, ciphers, IdentityCipherSuiteFilter.INSTANCE, toNegotiator(
				toApplicationProtocolConfig(nextProtocols), false), sessionCacheSize, sessionTimeout);
	}
	 ...
	 * @param apn
	 *             Provides a means to configure parameters related to
	...
	public JdkSslClientContext(File certChainFile, TrustManagerFactory trustManagerFactory, Iterable<String> ciphers,
			...
			throws SSLException {
		this(certChainFile, trustManagerFactory, ciphers, cipherFilter, toNegotiator(apn, false), sessionCacheSize,
				sessionTimeout);
	}
	...
	@Deprecated
	public JdkSslClientContext(File certChainFile, TrustManagerFactory trustManagerFactory, Iterable<String> ciphers,
			CipherSuiteFilter cipherFilter, JdkApplicationProtocolNegotiator apn, long sessionCacheSize,
			long sessionTimeout) throws SSLException {
		this(certChainFile, trustManagerFactory, null, null, null, null, ciphers, cipherFilter, apn,
	 ...
	 *             the {@link TrustManagerFactory} that provides the
	 *             {@link TrustManager}s that verifies the certificates sent
	 *             from servers. {@code null} to use the default or the results
	 *             of parsing {@code trustCertCollectionFile}
	 * @param keyCertChainFile
	 *             an X.509 certificate chain file in PEM format. This provides
	 *             the public key for mutual authentication. {@code null} to
	 *             use the system default
	 * @param keyFile
	 *             a PKCS#8 private key file in PEM format. This provides the
	 *             private key for mutual authentication. {@code null} for no
	 *             mutual authentication.
	 * @param keyPassword
	 *             the password of the {@code keyFile}. {@code null} if it's
	 *             not password-protected. Ignored if {@code keyFile} is
	 *             {@code null}.
	 * @param keyManagerFactory
	 *             the {@link KeyManagerFactory} that provides the
	 *             {@link KeyManager}s that is used to encrypt data being sent
	 *             to servers. {@code null} to use the default or the results
	 *             of parsing {@code keyCertChainFile} and {@code keyFile}.
	 * @param ciphers
	 *             the cipher suites to enable, in the order of preference.
	 *             {@code null} to use the default cipher suites.
	 * @param cipherFilter
	...
	public JdkSslClientContext(File trustCertCollectionFile, TrustManagerFactory trustManagerFactory,
		...
		super(newSSLContext(toX509CertificatesInternal(trustCertCollectionFile), trustManagerFactory,
				...
				keyPassword, keyManagerFactory, sessionCacheSize, sessionTimeout), true, ciphers, cipherFilter,
				apn, ClientAuth.NONE, false);
	}
	...
	JdkSslClientContext(X509Certificate[] trustCertCollection, TrustManagerFactory trustManagerFactory,
		...
		super(newSSLContext(trustCertCollection, trustManagerFactory, keyCertChain, key, keyPassword,
				keyManagerFactory, sessionCacheSize, sessionTimeout), true, ciphers, cipherFilter, toNegotiator(
				apn, false), ClientAuth.NONE, false);
	}
