...
public class ServerNIOEndPoint implements ServerEndPoint {
	...
	private NIOException handleException(IOException exception) throws NIOException {
		this.endConnect = true;
	...
	public void write(byte b) throws NIOException {
		ByteBuffer buffer = ByteBuffer.allocate(1);
		buffer.put(b);
		write(buffer);
	}

	public void write(byte[] bytes) throws NIOException {
		write(ByteBuffer.wrap(bytes));
	}

	public void write(byte[] bytes, int offset, int length) throws NIOException {
		write(ByteBuffer.wrap(bytes, offset, length));
	}

	public void write(ByteBuffer buffer) throws NIOException {
		write(channel, buffer);
	}

	// TODO 网速比较慢的时候
	private void write(SocketChannel channel, ByteBuffer buffer) throws NIOException {
