...
public class ContactServlet extends LMServlet {
	...
	public static final String	ACTION_GETCONTACTLISTBYUSERID= "getContactListByUserID";
	...
	protected void doAccept(IOSession session, ServerReadFuture future, AbstractService service) throws Exception {
		...
		ContactService userService = (ContactService) service;
		...
		if (ACTION_GETCONTACTLISTBYUSERID.equals(action)) {
			getContactListByUserID(session, future, parameters, userService);
		} else {
	...
	private void getContactListByUserID(IOSession session, ServerReadFuture future, Parameters parameters, ContactService userService)
			throws Exception {

		Integer userID = session.getAuthority().getUserID();
		
		List<B_Contact> contactList = userService.getContactListByUserID(userID);
