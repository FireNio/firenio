...
public class ContactService extends AbstractService {
	...
	public List<B_Contact> getContactListByUserID(Integer userID) throws SQLException {
		...
		List<B_Contact> list = (List<B_Contact>) query
				.query("select u.userID,u.nickname,u.phoneNo ,c.groupID,cg.groupName from t_contact c left join t_user u on c.friendID = u.userID left join t_contact_group cg on c.groupID = cg.groupID where c.ownerID = ?",
